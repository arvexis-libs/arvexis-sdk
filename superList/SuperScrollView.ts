...
export class SuperScrollview extends ScrollView {
    ...
    @property({
        tooltip: ",horizontalverticalverticalhorizontalhorizontalvertical "
    ...
    protected _updateScrollBar(outOfBoundary: any) {
        ...
        let offset = this.vertical ? outOfBoundary.y : -outOfBoundary.x
        if (offset > 0) { // 
            let progress = offset < EPSILON ? 0 : offset / this.headerOutOffset // headerOutOffset 
            ...
            let progress = -offset < EPSILON ? 0 : -offset / this.footerOutOffset  // footerOutOffset 
